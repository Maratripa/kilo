...
int getCursotPosition(int *rows, int *cols) {
    char buf[32];
    unsigned int i = 0;
...
int getWindowSize(int *rows, int *cols) {
    ...
    if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
        ...
        editorReadKey();
        return getCursotPosition(rows, cols);
    } else {
        *cols = ws.ws_col;
        *rows = ws.ws_row;
        return 0;
...
void editorDrawRows(struct abuf *ab) { // TODO Change welcome message
    int y;
...
void editorDrawStatusBar(struct abuf *ab) { // TODO Display column number
